...
import { PaperPlaneTilt, Sparkle, ArrowsOut, ArrowsIn, FlowArrow } from '@phosphor-icons/react'
...
export const OperationsCopilot: React.FC = () => {
  ...
  const { state: streamState, connect: streamConnect, disconnect: streamDisconnect } = useCopilotStream()
  ...
  }, [streamState.stage, streamState.response, streamState.error, addToast, isLoading])
  ...
  }, [addToast, streamConnect, useStreaming, auth])
        ...
        <div className="p-4 border-t border-steel-grey/30 bg-abyss/80 rounded-b-xl">
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' && !isLoading && !streamingActive) handleQuery(input) }}
              ...
              disabled={isLoading || !!streamingActive}
              aria-label="Ask a question"
            />
            <button
              onClick={() => handleQuery(input)}
              disabled={isLoading || !!streamingActive || !input.trim()}
